...
namespace WebChat.Models.Repositories.Implementation
{
    public class ChatRepository : IChatRepository
    {
        ...
        public IEnumerable<Message> GetMessages(int chatId, int take = 100, int skip = 0)
            => //context.Chats
               // .Include(c=>c.History)
               // .FirstOrDefault(c => c.ChatID == chatId)
               //     ?.History.SkipLast(skip).TakeLast(take);
               context.Messages
                .Include(m=>m.Chat)
                .Include(m=>m.From)
        ...
        public void AddBotToChat(string name, int chatId)
        {
        ...
        public void RemoveBotFromChat(string name, int chatId)
        {
            var bot = context.Bots.FirstOrDefault(b => b.Name == name);
            if (bot != null)
            {
